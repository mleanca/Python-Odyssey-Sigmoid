...
class Sistem:
    ...
    def adauga_utilizator(self, user):
        if not isinstance(user, user):
            raise TypeError("Argumentul `user` trebuie să fie de tip `user`.")

        self.__id_curent += 1
        self.__utilizatori[self.__id_curent] = user

    def afiseaza_utilizatori(self):
        return [user.get_nume() for user in self.__utilizatori.values()]

    def verifica_nivel_acces(self, nume_utilizator):
        for user in self.__utilizatori.values():
            if user.get_nume() == nume_utilizator:
                return user.get_nivel_acces()
        raise KeyError(f"Nu a fost găsit un utilizator cu numele '{nume_utilizator}'.")

    def modifica_nume_user(self, id_user, nume_nou):
        if id_user not in self.__utilizatori:
            raise KeyError(f"Nu a fost găsit un utilizator cu ID-ul {id_user}.")
        self.__utilizatori[id_user].set_nume(nume_nou)

def sterge_utilizator(self, id_user):
    if id_user not in self.__utilizatori:
        raise KeyError(f"Nu a fost găsit un utilizator cu ID-ul {id_user}.")

    del self.__utilizatori[id_user]

def modifica_nivel_acces(self, id_user, nivel_acces_nou):
    if id_user not in self.__utilizatori:
        raise KeyError(f"Nu a fost găsit un utilizator cu ID-ul {id_user}.")

    if nivel_acces_nou not in ("Default", "Manager", "Admin"):
        raise ValueError(f"Nivelul de acces nou '{nivel_acces_nou}' nu este valid.")

    self.__utilizatori[id_user].set_nivel_acces(nivel_acces_nou)
# CODUL TĂU VINE MAI SUS:
